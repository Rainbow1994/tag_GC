...
#temp_res = gcdf.temporal_resolution  # for future use
...
#    print('-'*10+'istep_start, istep_end:' istep, istep_end)
        ...
        for isub_run  in range(nsub_run): 
            ...
            if (istep in gcdf.select_runs):
                ...
                if (new_restart):
                    ...
                    os.system('cp '+ run_path+'geos.mp  ./'  )
                    new_restart = False
                else:
                    og_enaf = r'ST%3.3d.EN%4.4d-EN%4.4d' % (istep-1, mst, mend)
                    og_restart_file = gcdf.res_path+'Restart.'+og_enaf+'.'+tst[0:8]+'_0000z'+'.nc4'
                    os_cp_cmd = 'cp '+og_restart_file+' '+ gcdf.res_path + full_restart_name
                    os.system(os_cp_cmd)
 ...
 #               print('re_run_now, tst, re_run_date',re_run_now, tst, re_run_date)
                ...
                print('original Restart file, new Restart file:',gcdf.res_path+full_restart_name,'./'+full_restart_name )
                ...
                #>>>>> lauch the CTM 
#                os.system('nohup ./geos.mp > myout.file 2>&1 &')
                os.system('./geos.mp')   

...
!
	...
	! #levels of TAGCH4 MASK NAMELY The NUMBER OF MASK REGIONS ADDED BY RAINBOW
    LS        = Input_Opt%Max_Regions 
    ...
    State_Diag%Archive_TAGCH4_MASK                 = .FALSE.
 ...
 10 FORMAT( /, 'Allocating the following fields of the State_Diag object:' )
    ...
    IF ( Input_Opt%ITS_A_CH4_SIM ) THEN
	   ...
	   !--------------------------------------------------------------------
       ! TAGCH4_MASK for tagCH4 simulation
       !--------------------------------------------------------------------
       arrayID = 'State_Diag%TAGCH4_MASK' 
       diagID  = 'TAGCH4_MASK'
       CALL Check_DiagList( am_I_Root, Diag_List, diagID, Found, RC )
       IF ( Found ) THEN
          IF ( am_I_Root ) WRITE( 6, 20 ) ADJUSTL( arrayID ), TRIM( diagID )
          ALLOCATE( State_Diag%TAGCH4_MASK(IM,JM,LS), STAT=RC )
          CALL GC_CheckVar( arrayID, 0, RC )
          IF ( RC /= GC_SUCCESS ) RETURN
          State_Diag%TAGCH4_MASK = 0.0_f4
          State_Diag%Archive_TAGCH4_MASK = .TRUE.
          CALL Register_DiagField( am_I_Root, diagID,                        &
                                   State_Diag%TAGCH4_MASK,                &
                                   State_Chm, State_Diag, RC )
          IF ( RC /= GC_SUCCESS ) RETURN
       ENDIF
...
!
   ...
   ENDIF
	...
	IF ( ASSOCIATED( State_Diag%TAGCH4_MASK ) ) THEN
       DEALLOCATE( State_Diag%TAGCH4_MASK, STAT=RC )
       CALL GC_CheckVar( 'State_Diag%TAGCH4_MASK', 2, RC )
       IF ( RC /= GC_SUCCESS ) RETURN
       State_Diag%TAGCH4_MASK => NULL()
    ENDIF
...
!
	...
	ELSE IF ( TRIM( Name_AllCaps ) == 'TAGCH4_MASK' ) THEN
       IF ( isDesc    ) Desc  = 'MASK for TAGCH4 SIMULATION'
       IF ( isUnits   ) Units = '1'
       IF ( isRank    ) Rank  =  3 

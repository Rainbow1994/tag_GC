...
!                              deposition velocity for sulfate aerosols!  18 Nov 2015 - M. Sulprizio- Add passive tracers PASV to RnPbBe simulation
...
!
    ...
    ! For Tagged Run (Rainbow)
    INTEGER             :: TagIdx
    INTEGER             :: TagIdx_Spec
    LOGICAL             :: Is_Tagged           ! Denotes TAG species    
...
!
! !DEFINED PARAMETERS:
	! DETEMINE THE INDEX OF TAGSPECIES(Rainbow) 
	INTEGER             :: TagIdx
 	INTEGER             :: TagIdx_Spec								  									
!
    ! Local parameter
    ...
    SELECT CASE( TRIM ( Name ) )
          ...
          CASE DEFAULT
             ...
             IsPassive = .FALSE.
             ! Check if tag species(Rainbow)
             Is_Tagged = .FALSE.
             ! Determine whether the name of speices contains 'TAG'(Rainbow)
             TagIdx = INDEX(Name, 'TAG')
             ...
             ! Create tagCH4 species by Rainbow
             ELSEIF ( TagIdx > 0 ) THEN     
                !------------------------------------------------------------
                ! JUST ADD A NEW TAGRUN SPECIES IN THE SPECIES_DATABASE
                ! create entry in the species database
                !------------------------------------------------------------
                TagIdx_Spec = INDEX( Name, 'CH4')
                IF( TagIdx_Spec > 0) THEN
                         FullName        = TRIM( Name )
                         Formula         = 'CH4'
                         Mw_g            = 16.0_fp
                         Is_Gas          = T

...
!
  ...
  TYPE, PUBLIC :: OptInput
	 ...
	 INTEGER                     :: NYMDr
     INTEGER                     :: NHMSr
	 INTEGER 				     :: TAG_CH4_SOUS
	 INTEGER 				     :: TAG_CH4_REGS
	 CHARACTER(LEN=255), POINTER :: Source_Name(:)													 
     !----------------------------------------
     ! POPS MENU fields
     !----------------------------------------
...
!
	...
	Input_Opt%Max_Sources            = 50											
	Input_Opt%Max_Regions            = 300
    !----------------------------------------
    ! SIMULATION MENU fields
    !----------------------------------------
    Input_Opt%NYMDb                  = 0
    ...
    ! CH4 MENU fields
    !----------------------------------------
	arrayId = 'Input_Opt%Source_Name'
    ALLOCATE( Input_Opt%Source_Name( Input_Opt%Max_Sources ), STAT=RC )
    CALL GC_CheckVar( arrayId, 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
	...
	Input_Opt%NYMDr                  = 0
    Input_Opt%NHMSr                  = 0									 										
	Input_Opt%TAG_CH4_SOUS           = 0
    Input_Opt%TAG_CH4_REGS           = 0
	Input_Opt%Source_Name            = ''												 
    !----------------------------------------
    ! POPS MENU fields
    !----------------------------------------
    Input_Opt%POP_TYPE               = ''
...
!BOC
	...
	IF ( ASSOCIATED( Input_Opt%Source_Name ) ) THEN
       DEALLOCATE( Input_Opt%Source_Name, STAT=RC )
       CALL GC_CheckVar( 'Input_Opt%Source_Name', 2, RC )
       IF ( RC /= GC_SUCCESS ) RETURN
       Input_Opt%Source_Name => NULL()
    ENDIF
#endif

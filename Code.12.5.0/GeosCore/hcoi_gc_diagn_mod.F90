...
!
	...
	!Added by Rainbow at Nov.27 2019
    INTEGER            :: I_SOUS, I_REGS 
    INTEGER            :: n_SOUS, n_REGS, I_SPC	
    CHARACTER(LEN= 63),ALLOCATABLE :: DgnName_pre(:)
    ...
    ExtNr = 0
	! EMISSION CATEGORY AND REGION NUMBERS
	n_SOUS		 = Input_Opt%TAG_CH4_SOUS
  	n_REGS       = Input_Opt%TAG_CH4_REGS
	DgnName_pre  = Input_Opt%Source_Name
	
	!----------------------------------------------------------------- 
	! %%%%%READ TAGCH4 DIAGNOSTICS INFORMATION  ADDED BY RAINBOW %%%%%
	!-----------------------------------------------------------------
	!emission source loop
!	write(6,*) n_SOUS, n_REGS 
	DO I_SOUS = 1, n_SOUS 
		!DgnNames need to be same as EMIS_'NAME' in HEMCO_Diagn.rc 

		! region division loop
		DO I_REGS =1, n_REGS 
			WRITE( cTemp,'(i3)' ) I_REGS	
			DiagnName =  trim(DgnName_pre(I_SOUS))//trim(adjustl( cTemp ))
			Cat = (I_SOUS-1) * n_REGS + I_REGS
			!write(6,*) I_SOUS, I_REGS, Cat, DiagnName
			!Check if tagged CH4 simulation
			HcoID = HCO_GetHcoID( DiagnName, HcoState )
			IF ( HcoID <= 0 ) THEN
				HcoID = id_CH4
			ENDIF
